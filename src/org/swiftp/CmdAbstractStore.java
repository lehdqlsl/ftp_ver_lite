...
abstract public class CmdAbstractStore extends FtpCmd {
	...
	static Handler listmsg;
	static Handler sizemsg;
	static Handler UImsg;
	public static void ListmsgsHandle(Handler handle, Handler handle2)
	{
		listmsg = handle;
		UImsg = handle2;
	}
	...
	public void doStorOrAppe(String param, boolean append) {
		...
		//myLog.l(Log.INFO, "file size!! : " + storeFile.length());
...
//		myLog.l(Log.DEBUG, "STOR original priority: " + origPriority);
		storing: {
			...
			try {
				if(storeFile.exists()) {
					double bytes = storeFile.length();
					myLog.l(Log.INFO, "file size : " + bytes);
					if(!append) {
						if(!storeFile.delete()) {
...
//			}
		...
		if(errString != null) {
			...
			UImsg.sendEmptyMessage(4);
			//리스트 메세지 핸들러		
			Message msg = listmsg.obtainMessage();
			Bundle data = new Bundle();
			data.putString("data", storeFile.getName()+":"+storeFile.length());
			msg.setData(data);
			listmsg.sendMessage(msg);
			myLog.l(Log.INFO, "메세지보냄: " + storeFile.getName()+":"+storeFile.length());
		}
